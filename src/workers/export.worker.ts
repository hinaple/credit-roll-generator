...
import { paintCreditsFrame } from '../lib/core/render';
import type { CreditConfig, CreditLayout, FramePlan } from '../lib/core/types';
...
async function loadFonts(layout: CreditLayout): Promise<void> {
  if (!layout.fonts.length) return;
  if (typeof FontFace === 'undefined' || !scope.fonts) throw new Error('이 브라우저의 worker에서는 사용자 글꼴을 사용할 수 없습니다.');
...
scope.onmessage = async ({ data }) => {
  ...
  const imageSources = new Map<string, ImageBitmap>();
  try {
    const { config, layout, plan, audio, images } = data;
    ...
    for (const [id, bitmap] of loadedImages) imageSources.set(id, bitmap);
    ...
    const source = new CanvasSource(canvas, { codec: 'avc', bitrate: videoBitrate });
    output.addVideoTrack(source, { frameRate: config.fps });

    let audioSource: AudioSampleSource | null = null;
    if (audio && config.audioStart < plan.total && audio.numberOfFrames > 0) {
    ...
    if (audioSource && audio) {
      scope.postMessage({ type: 'status', message: '음성 인코딩 중...' });
    ...
    for (let frame = 0; frame < plan.totalFrames; frame++) {
      paintCreditsFrame(context, config, layout, plan, frame, imageSources);
      await source.add(frame * frameDuration, frameDuration, { keyFrame: frame % keyInterval === 0 });
      if (frame % 5 === 0 || frame + 1 === plan.totalFrames) {
        scope.postMessage({ type: 'progress', frame: frame + 1, total: plan.totalFrames });
      }
    }
    ...
    scope.postMessage({ type: 'done', buffer: target.buffer }, [target.buffer]);
  } catch (error) {
    scope.postMessage({ type: 'error', message: error instanceof Error ? error.message : String(error) });
  } finally {
    for (const bitmap of imageSources.values()) bitmap.close();
  }
};
